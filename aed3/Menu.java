...
public class Menu {
  ...
  private int Acesso()throws Exception  {
  ...
  public Usuario SignIn() throws Exception {
  	Usuario user = new Usuario();
  	System.out.println("\nNOVO USUARIO\n============\n\nEmail: ");
  	String NovoEmail = sc.nextLine();
   	if(au.read(NovoEmail) != null)  System.out.println("Email já Cadastrado!");
    else {
  ...
  private Usuario TrocarConta() throws Exception{
    ...
    if(user!=null) {
      if(TesteSenha(user)){ 
        if (TesteFrase(user)) {user=UpdateConta(user);}
      }
      else {
        for(int i = 0; i < 2; i++) {
  ...
  public int MinhasPerguntas() throws Exception {
    int escolha;
    System.out.println("\nPERGUNTAS 1.0\n=============\n\nINÍCIO > MINHA ÁREA > MINHAS PERGUNTAS\n1) Listar\n2) Incluir\n3) Alterar \n4) Arquivar \n\n0) Sair\n\nOpção: ");
    try {
      escolha=Integer.parseInt(sc.nextLine());
    } catch (Exception e) {escolha=-1;}
    switch (escolha) {
      case 1:
        escolha=-1;
        ListarPerguntas();
  ...
  public void InserirPergunta() throws Exception{
    ...
    if(pergunta == null || pergunta.equals("")){
      InserirPergunta();
    }else{  if(pergunta.length()==1 && pergunta.charAt(0) == '0') return;
